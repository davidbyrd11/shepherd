...
Builder.prototype._setPrioritiesRecursive = function (node, importantBonus, visited) {
  ...
  for (i = 0; i < node.importantInputs.length; i++) {
    ...
    if (inputNode) {
      ...
      if (inputNode.raisePriority(node.getPriority() + importantBonus) ||
          !visited[inputNode.newName]) {
  ...
  for (i = 0; i < node.argInputs.length; i++) {
    ...
    inputNode = this._compiled.nodes[inputRootName]
    if (inputNode) {
      if (inputNode.raisePriority(node.getPriority()) ||
          !visited[inputNode.newName]) {
        this._setPrioritiesRecursive(inputNode, importantBonus, visited)
      }
    }
...
Builder.prototype._addImportantDependenciesForNode = function (node, deps, visited) {
  ...
  for (i = 0; i < deps.length; i++) {
    ...
    if (!found && depNode.getPriority() > node.getPriority()) {
      node.inputs[depName] = depName
      node.importantInputs.push(depName)
      node.implicitInputs.push(depName)
      if (node.ignoredErrors[utils.DYNAMIC_NODE_REF]) {
        node.ignoredErrors[depName] = true
      }
      changed = true
    }
  }
  ...
  for (i = 0; i < node.importantInputs.length; i++) {
    ...
    inputNode = this._compiled.nodes[inputRootName]
    if (inputNode) {
      if (node.implicitInputs.indexOf(node.importantInputs[i]) === -1) {
